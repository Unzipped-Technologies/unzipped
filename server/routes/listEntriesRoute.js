...
router.post('/users-list-entries', async (req, res) => {
  try {
    const listEntries = await ListEntriesHelper.getUserListEntries(req.body)
    res.send(listEntries)
  } catch (error) {
    console.log('Error on retrieve users list entries'.error)
    res.status(500).send(error)
  }
})
