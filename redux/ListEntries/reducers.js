...
const ListEntries = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case GET_USER_LIST_ENTRIES: {
      return { ...state, loading: false, userListEntries: [...action?.payload] }
    }

    case GET_USER_LIST_ENTRIES_ERROR:
      return { ...state, loading: false, userListEntriesError: action.payload }
