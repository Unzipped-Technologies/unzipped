...
`
const VerifyEmail = ({ loading, token, userForm, error, isEmailSent, updateRegistrationCredentials, verifyUser, }) => {
        ...
        <>
            <Container>
                <InnerContainer>
                    ...
                    <p className='mt-5 mb-3'><b>VERIFY YOUR EMAIL TO PROCEED</b></p>
                    ...
                    <Button colors={{ text: '#4285F4', border: 'transparent' }} background='transparent' onClick={() => { setSowEmailChange(!showEmailChange) }}>
                        <EmailDiv gap="3px">
                    ...
                    <br />
                    {showEmailChange && <Form>
                        <Input type='email' placeholder='UPDATE EMAIL' value={email} onChange={(e) => setEmail(e.target.value)} required></Input>
                        <Button colors={{ text: '#FFFFFF', border: '#E25050' }} background='#E25050' oval small onClick={handleSubmit}>UPDATE</Button>
                    </Form>}
                    {!loading && isEmailSent && <P>Verification email has been sent!</P>}
                    <EmailDiv gap='20px'>
                        <Button colors={{ text: '#37DEC5', border: '#37DEC5' }} background='transparent' oval onClick={() => { handleRequest() }} disabled={loading}>{loading && <CircularProgress size={18} />}  Resend verification email</Button>
                        <Button colors={{ text: '#363636', border: '#37DEC5' }} background='#37DEC5' oval > <a href="https://mail.google.com/mail/u/0/?pli=1#inbox" target="_blank" rel="noopener noreferrer">
                            Go to Google Inbox
                        </a></Button>
                    </EmailDiv>
                </InnerContainer>
