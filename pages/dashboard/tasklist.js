    ...
    createStory(
      {
        ...
        assigneeId:
          ...
          employees.find(
            item =>
              item?.FirstName.toLowerCase().includes(form?.assignee.toLowerCase().split(' ')[0]) &&
              item.LastName.toLowerCase().includes(form?.assignee.toLowerCase()?.split(' ')[1])
          )?._id,
      ...
      <ListPanel
        ...
        list={departments.map(e => {
          return {
            ...
            text: e?.name,
            icon: (
              <Icon name={e.isSelected || e?._id === selectedDepartment?._id ? 'selectedDepartment' : 'department'} />
            ),
            padding: true
          }
        })}
