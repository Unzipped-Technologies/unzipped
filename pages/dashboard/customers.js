...
import { getCustomers, loadMoreCustomers, addCustomer } from '../../redux/actions';
...
const Customers = ({users, token, cookies, userType, loading}) => {
    ...
    const checkAllUsers = () => {
        ...
        const checked = users.map(item => ({...item, checked: !checkAll}))
        setChecked(checked);
    }

    const grabChecked = () => {
        const checked = users.map(item => ({...item, checked: false}))
        setChecked(checked);
    }
                ...
                <div className="right-dashboard">
                    ...
                    <div className="dashboard-main">
                        ...
                        <div className="card-title" id="promo-title">
                                <h4 className="mobile-dashboard-title" id="customer-dash">Customers</h4>
                                <div className="button-mobile-mover">
                                    <AddCustomer open={open} setOpen={setOpen} token={access}/>
                                    <DeleteCustomer open={deleteOpen} setOpen={setDeleteOpen} token={access} deletes={deletes}/>
                                    ...
                                    <button className="customer-btn" onClick={() => setOpen(true)}><i className="fa fa-plus" style={{fontStyle: 'normal', marginRight: '7px', color: '#fff', fontWeight: '200'}}/>Add customer</button>
                                    <button className="customer-btn" onClick={() => setFocus('more')} id="more-btn">
                                        ...
                                        {focus === "more" && 
                                            <div className="more-box" ref={wrapperRef}>
                            ...
                            <div className="row-box-customer">
                                <div className="first-row" id="customer-first-row">
                                    <div className="first-row" id="left-first-customer">
                                        <div className="left-box" id="email-title-customer">
                                            ...
                                            />
                                            <p className="email-customer bold" >EMAIL</p>
                                        </div>
                                        <p className="description-customer bold description-mobile">DESCRIPTION</p>
                                    </div>
                                    <div className="first-row column-two-customer bold" id="slide-shift-customer">
                                        <p id="move-text-type" id="customer-col-2" style={{width: '70px'}}>USER TYPE</p>
                                        <p id="move-text-customer" className="customer-mobile-created">CREATED</p>
                                    </div>
                                </div>
                            ...
                            {checked.map((item, index) => {
                                return (
                                    <div key={index} className="user-rows">
                                        ...
                                        <div className="left-box email-customer-check">
                                            <Checkbox
                                                color="primary"
                                                ...
                                                name={item._id}
                                            />
                                        <p className="email-customer">{item.email.substring(0,26)}</p>
                                        </div>
                                        <p className="description-customer description-mobile">-</p>
                                        </div>
                                        <div className="first-row column-two-customer" id="slide-shift-customer">
                                            <p id="customer-col-3">{item.userType}</p>
                                            <p className="customer-mobile-created">{item.dateCreated.substring(0,15)}</p>
                                        </div>
                                    </div>
                                )
                            })}
...
export default connect(mapStateToProps, {getCustomers, loadMoreCustomers, addCustomer})(Customers);