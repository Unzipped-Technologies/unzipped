...
const Orders = ({token, cookies, orders, loading, hasMoreOrders, userType, users}) => {
    ...
    const printPdf = () => {
        const printPdf = Axios
            .post(`/api/dashboard/orders/pdf`, {status: "PENDING"}, { responseType: 'blob' })
            .then((res) => {
    ...
    const grabChecked = () => {
        const checked = orders.map(item => ({...item, checked: false}))
        setChecked(checked);
    }
                                ...
                                return (
                                    <div key={index} className="user-rows">
                                        ...
                                        <div className="first-row column-two-customer">
                                            <p className="order-total-mobile">${item.total.toFixed(2)}</p>
