...
describe('DesktopAccount Component', () => {
  ...
  it('renders Tasklist, click on Add Task for todo and verify input fields are rendering', async () => {
    ...
    const assigneeField = within(TaskForm).getByTestId('assignee')
  ...
  it('renders Tasklist, click on Add Task for todo and submmit new task with success respose', async () => {
    ...
    // // Assignee
    const AssigneeField = within(TaskFormContainer).getByTestId('assignee')
    expect(AssigneeField).toBeInTheDocument()
    ...
    const Option1 = within(AssigneeField).getByText('Unassigned')
    expect(Option1).toBeInTheDocument()
    fireEvent.click(Option1)

    const Option2 = within(AssigneeField).getByText(OptionLabel.freelancer.user.email)
    expect(Option2).toBeInTheDocument()
    fireEvent.click(Option2)

    initialState.Tasks.createStoryForm.assignee = OptionLabel?.freelancerId

    fireEvent.change(AssigneeField, { target: { value: OptionLabel?.freelancerId } })
  ...
  it('renders Tasklist, click on task and edit it successfully', async () => {
    initialState.Tasks.createStoryForm.businessId = initialState.Tasks.selectedTask.business._id
    initialState.Tasks.createStoryForm.departmentId = initialState.Tasks.selectedTask.department._id
