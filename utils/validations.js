...
class Validations {
  ...
  checkNullOrEmpty(list, obj) {
    ...
    if (obj.investorType === 'LLC' && list.includes('entityName')) {
      if ([1, 2, 3, 4, 5].indexOf(parseInt(obj.investorSubType)) === -1) {
        const errorCondition1 =
          ...
          errorCondition2 =
            obj.countryResidenceId === 231 && (!obj.stateResidenceId || parseInt(obj.stateResidenceId) === 0)
    ...
    const noMailingCountry = !list.includes('mailingAddressCountry'),
      ...
      countryError =
        parseInt(obj.countryResidenceId) === 231 && (parseInt(obj.stateResidenceId === 0) || !obj.stateResidenceId)
  ...
  _strongPasswordValidation(pw) {
    return (
      /[A-Z]/.test(pw) && // At least one uppercase letter
