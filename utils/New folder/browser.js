...
class BrowserUtil {
    ...
    getLpFundId() {
        ...
        if (
            urlSplitFundId.indexOf('&envelope_id=') === -1 &&
            urlSplitFundId.indexOf('type=') === -1 &&
            urlSplitFundId.indexOf('id=') === -1
        ) {
            return parseInt(urlSplitFundId);
        } else {
            var id = urlSplitFundId.split('&')[0];
            return FsnetUtil._decrypt(id);
        }
    }
    ...
    _getId() {
        ...
        return urlSplitFundId !== 'funddetails' ? FsnetUtil._decrypt(urlSplitFundId) : 0;
    }
    ...
    async getToken() {
        if (UserUtils.isWithoutAccount()) {
            return UserUtils.getItem('token');
        }
