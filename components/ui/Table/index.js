...
import { Action, Actions, Button, Icon } from '../../ui'
import useSharedWidth from '..//hooks/useSharedWidth'
...
const Filter = ({ sort, theadData, column, index }) => {
  ...
  return (
    <FilterIcons data-testid="action-table-filter-icon">
      <Icon name="filterup" fill={sort === 'up' && column === label.toLowerCase() ? theme.secondary : theme.tint2} />
      <Icon
        name="filterdown"
...
const Table = forwardRef(
  (
    ...
    const allCheckedValues = useMemo(() => new Set(customCheckedValues.map(item => item.id)), [customCheckedValues])
              ...
              <FullWidthContainer>
                ...
                <ScrollableWrapper isPadding={isPadding}>
                  <SimpleBar data-simplebar>
                    <div>
                      <Content>
                        <FullWidthTHead type={type} isFullWidth={isFullWidth} $isEmpty={isEmpty}>
                          {theadData.slice(hasChecksOrRadios(type) ? 0 : 1).map((header, index) => {
                            return (
                              <TableHead
                                key={header + index}
                                item={header}
                                ...
                                sort={sort}
                                column={column}
                                width={columnWidths[hasChecksOrRadios(type) ? index + 1 : index + 1]}
                                isMiddleCol={true}
                                upperCaseHeadings={upperCaseHeadings}
                              />
                            )
                  ...
                  <>
                    ...
                    <RightSidebar hasMultipleActions={hasMultipleActions}>
                      <THead type={type} isFullWidth={isFullWidth} $isEmpty={isEmpty}>
                        <TItem right isSingleAction={!hasMultipleActions} singleActionTitle={!hasMultipleActions}>
