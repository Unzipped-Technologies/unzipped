...
const FullScreenDropdown = ({ menuItems, onClose, isAuth = false, logoutUser, startAProject, role }) => {
  ...
  return (
    <Container>
      <Header>
        ...
        <Magnify className="material-icons">search</Magnify>
      </Header>
      <ItemContainer>
                ...
                <SubContainer>
                  ...
                  {item.subItems.map((sub, index) => {
                    return (
                      <SubItem
                        key={sub.name + index}
                        onClick={() => typeof sub.sub === 'string' && linkPush(sub.link)}
                        sub={!(typeof sub.sub === 'string')}>
                        <SubItemTitle>{sub.name}</SubItemTitle>
                        <SubItemDescription sub={!(typeof sub.sub === 'string')}>
                          {typeof sub.sub === 'string'
                            ? sub.sub
                            : sub.sub.map((element, index) => {
                                return (
                                  <WikiContainer key={element.name + index} onClick={() => linkPush(element.link)}>
                                    {element.icon}
                                    <WikiTextBox>
                                      <WikiTitle>{element.name}</WikiTitle>
                                      <WikiSubTitle>{element.sub}</WikiSubTitle>
                                    </WikiTextBox>
                                  </WikiContainer>
                                )
                              })}
      ...
      <Footer>
        {isAuth ? (
          <TopButton extraWide={true} onClick={logoutUser}>
            Log Out
          </TopButton>
        ) : (
          <TopButton onClick={() => linkPush('/login')} extraWide={true}>
            Log In
          </TopButton>
        )}
        {role !== 1 && (
