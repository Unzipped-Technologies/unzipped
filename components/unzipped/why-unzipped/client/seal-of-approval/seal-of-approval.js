  ...
  return (
    <>
      <Container>
        <TextBoxContainer>
          <ParagraphText>
            You’ll have plenty of help chosing the right person for the job. And no matter who you’re hiring you can:
          </ParagraphText>
        </TextBoxContainer>
        <ChecklistContainer>
          {checkList.map((item, index) => (
            <ParagraphTextList key={index}>
              {' '}
              <Span>{item.icon}</Span> {item.description}{' '}
            </ParagraphTextList>
          ))}
        </ChecklistContainer>
      </Container>
