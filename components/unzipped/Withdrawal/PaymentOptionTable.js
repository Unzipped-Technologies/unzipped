...
const TableCell = styled.div`
  ...
  min-width: ${({ isMethod, isFee }) =>
    isMethod ? '160px' : isFee ? '50px' : '0'}; // Set a min-width for first and last column
...
export const withdrawalMethods = [
  {
    ...
    description:
      'The fastest method to withdraw funds, directly to your local bank account! Available in selected countries only.',
    fee: 'No Fee'
  },
  {
    method: 'Payoneer',
  ...
  return (
    <Table id="payment_options_table">
      ...
      {withdrawalMethods.map((method, index) => (
        <TableRow key={index}>
          <TableCell id="method" isMethod>
            {method.method}
          </TableCell>
          <TableCell id="description" isDescription>
            {method.description}
          </TableCell>
          <TableCell id="fee" isFee>
            {method.fee}
          </TableCell>
        </TableRow>
      ))}
