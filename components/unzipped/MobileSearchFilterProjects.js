...
function MobileSearchFilterProjects({ handleProjectTypes, handleFilterOpenClose, maxRate, setMaxRate, setMinRate, minRate, handleSearch, freelancerSkillsList, setSkill }) {
        ...
        <div style={{ backgroundColor: "white", color: "black" }}>
            ...
            <div style={{ padding: "0 25px", paddingTop: "72px" }}>
                ...
                {Object.keys(projectTypes).map((type, index) => {
                    return (
                        <div className='d-flex' onClick={() => handleProjectType(type)} >
                            {projectTypes[type] ? (
                                <CheckBox backgroundColor="rgba(102, 102, 102, 0.1)" >
                                    <span style={{ display: "flex", justifyContent: "center", fontSize: "smaller", fontWeight: "bolder" }}>✓</span>
                                </CheckBox>
                            ) : (
                ...
                <div className='d-flex justify-content-between' style={{ gap: "20px" }}>
                    <div>
                        ...
                        <div className='d-flex align-items-center border border-dark'>
                            ...
                            <input ref={minRef} type='number' value={minRate} name='min' onBlur={handleBlur} onChange={(e) => { setMinRate(e.target.value) }} style={{ margin: "0", border: "0", height: "37px" }}></input>
                        </div>
                    ...
                    <div>
                        ...
                        <div className='d-flex align-items-center border border-dark'>
                            <span style={{ padding: "0 18px 0 9px", display: "flex", alignSelf: "center" }}><svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 320 512" ><path d="M160 0c17.7 0 32 14.3 32 32V67.7c1.6 .2 3.1 .4 4.7 .7c.4 .1 .7 .1 1.1 .2l48 8.8c17.4 3.2 28.9 19.9 25.7 37.2s-19.9 28.9-37.2 25.7l-47.5-8.7c-31.3-4.6-58.9-1.5-78.3 6.2s-27.2 18.3-29 28.1c-2 10.7-.5 16.7 1.2 20.4c1.8 3.9 5.5 8.3 12.8 13.2c16.3 10.7 41.3 17.7 73.7 26.3l2.9 .8c28.6 7.6 63.6 16.8 89.6 33.8c14.2 9.3 27.6 21.9 35.9 39.5c8.5 17.9 10.3 37.9 6.4 59.2c-6.9 38-33.1 63.4-65.6 76.7c-13.7 5.6-28.6 9.2-44.4 11V480c0 17.7-14.3 32-32 32s-32-14.3-32-32V445.1c-.4-.1-.9-.1-1.3-.2l-.2 0 0 0c-24.4-3.8-64.5-14.3-91.5-26.3c-16.1-7.2-23.4-26.1-16.2-42.2s26.1-23.4 42.2-16.2c20.9 9.3 55.3 18.5 75.2 21.6c31.9 4.7 58.2 2 76-5.3c16.9-6.9 24.6-16.9 26.8-28.9c1.9-10.6 .4-16.7-1.3-20.4c-1.9-4-5.6-8.4-13-13.3c-16.4-10.7-41.5-17.7-74-26.3l-2.8-.7 0 0C119.4 279.3 84.4 270 58.4 253c-14.2-9.3-27.5-22-35.8-39.6c-8.4-17.9-10.1-37.9-6.1-59.2C23.7 116 52.3 91.2 84.8 78.3c13.3-5.3 27.9-8.9 43.2-11V32c0-17.7 14.3-32 32-32z" /></svg></span>
                            <input ref={maxRef} type='number' value={maxRate} name='max' onBlur={handleBlur} onChange={(e) => { setMaxRate(e.target.value) }} style={{ margin: "0", border: "0", height: "37px" }}></input>
                        </div>
                ...
                <div className='d-flex border border-dark mb-5' style={{ padding: "7.5px 20px", borderRadius: "7px", gap: "20px" }}>
                    ...
                    />
                    <ClearIcon onClick={()=>{setUserInput('');setSuggestions([])}} $show={userInput}>
                        <Icon name="closeBtn" />
                    </ClearIcon>
                </div>
                <div>
                ...
                {skillData?.map((skill) => (
                    <div className='d-flex' style={{ lineHeight: "normal" }}>
                        {skills.includes(skill?.skill) ? <CheckBox onClick={() => { setSkills(skills.filter((data) => data !== skill?.skill)); setUniqueSkills(prev => [...prev, skill]); setSkillData((prev) => [...prev.filter((data) => data.skill !== skill.skill)]) }} backgroundColor="rgba(102, 102, 102, 0.1)"><span style={{ display: "flex", justifyContent: "center", fontSize: "smaller", fontWeight: "bolder" }}>✓</span></CheckBox>
                            : <CheckBox onClick={() => {
                                setSkills((prev) => [...prev, skill?.skill]); setUniqueSkills(prev => [...prev.filter((data) => data.skill !== skill.skill)])
                            }} borderColor='rgba(102, 102, 102, 0.20)'></CheckBox>}
                        <p className='mx-3'>{skill?.skill}</p>
                    </div>
                ))}
                <div className='pb-3 pt-5' style={{ display: "grid" }}>
                    <button style={{ background: "#37DEC5", color: "white", fontSize: "18px", border: "0", padding: "10px 0px", fontWeight: "600" }} onClick={handleSearhButton}>SEE RESULTS</button>
                </div>
            </div>
        </div>
    )
