...
const SubscriptionCard = ({submitSubscription, disabled, plans, dateCode, subscriptionName, planCost, subscriptionForm, selectedPlan}) => {
    ...
    const total = (calcPrice(subscriptionForm?.paymentFrequency).toFixed(2) * (12 * subscriptionForm?.paymentFrequency > 0 ? 12 * subscriptionForm?.paymentFrequency : 1)).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')

    return (
        <Container>
            <WhiteCard height={(((selectedPlan + 1) * 50) + 450) + "px"}>
                <TitleText size="22px">{subscriptionName} Plan</TitleText>
                <DarkText >${total} + tax every {calcPaymentFrequency(subscriptionForm?.paymentFrequency)}</DarkText>
                <DarkText >You will be charged ${total} + tax on {dateCode}, when your trial ends.</DarkText>
                ...
                <ButtonContainer>
                    <Button disabled={disabled} noBorder block type="black" onClick={() => submitSubscription()}>START PLAN</Button>
                </ButtonContainer>
            </WhiteCard>
        </Container>
