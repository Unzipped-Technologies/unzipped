...
// Main Component
const ProjectKanbanBoard = ({ selectedDepartment, currentBusiness, businesses, isFullScreen }) => {
  ...
  useEffect(() => {
    if (searchTerm.length > 0) {
      const newFilteredTickets = { ...backendCols }
      Object.keys(newFilteredTickets).forEach(colKey => {
        const filteredTasks = newFilteredTickets[colKey].tasks.filter(
          task =>
            task?.taskName
              ?.toLowerCase()
              .trim()
  ...
  return (
    <>
      <GlobalStyle />
      <Container data-testid="kanban_container">
        <ProjectFilterContainer>
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <div
              ...
              <DIV>
                <div>
                  <AssignedToList ticketAssignedTo={assigneeFilters} setTicketAssignedTo={setAssigneeFilters} />
                </div>
                <div>
                  <Button
