...
const AssignedToList = ({ setTicketAssignedTo }) => {
  ...
  useEffect(() => {
    if (hiredProjectTeam && hiredProjectTeam.length > 0) {
      const finalArray = hiredProjectTeam.filter((item, index) => itemsIndex.includes(index))
    ...
    <>
      <AssignedToContainer>
        <div style={{ width: '100%', height: '100%' }}>
          <Button
            id="assignee_dropdown"
            aria-controls={open ? 'assignee_options' : undefined}
            aria-haspopup="true"
          ...
          <StyledMenu
            data-testid="assignee_options"
            id="assignee_options"
            MenuListProps={{
              'aria-labelledby': 'assignee_dropdown'
            }}
            anchorEl={anchorEl}
            open={open}
            onClose={handleClose}>
            {hiredProjectTeam &&
              hiredProjectTeam.map((member, index) => (
                <MenuItem
                  key={index}
                  id={`user_${index}`}
                  value={member}
                  onClick={() => handleMenuItemUserSelection(member, index)}>
                  {member?.userId && <Checkbox checked={itemsIndex.length > 0 ? itemsIndex.includes(index) : false} />}
