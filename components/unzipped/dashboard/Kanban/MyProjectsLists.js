    ...
    <>
      <Container>
        <CustomButton
          ...
          aria-controls={open ? 'projects_dropdown_menus' : undefined}
          aria-haspopup="true"
        ...
        <StyledMenu
          data-testid="projects_dropdown_menus"
          MenuListProps={{
            'aria-labelledby': 'projects_dropdown'
          }}
          anchorEl={anchorEl}
          open={open}
          onClose={handleClose}
                    ...
                    <List sx={{ margin: 0, boxShadow: 'none', border: 0 }}>
                      {project.businessDepartments.map((department, dptIndex) => (
                        <StyledListItem key={dptIndex} onClick={() => handleDepartment(project._id, department._id)}>
                          ...
                          <ListItemText primary={ConverterUtils.truncateString(department.name, 30)} />
                        </StyledListItem>
                      ))}
                    </List>
                  </AccordionDetails>
