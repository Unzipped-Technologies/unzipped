      ...
      {userListItems
        ...
        .map((item, index) => (
          <WhiteCard
            ...
            key={index}>
            {item?.isDefault && item.icon == 'FaEye' && <IconPickerItem icon={item.icon} size={24} color="#8EDE64" />}

            {item?.isDefault && item.icon == 'FaRegHeart' && (
              <IconPickerItem icon={item.icon} size={24} color="#FA00FF" />
            )}
            {item?.isDefault && item.icon == 'FaUsers' && <IconPickerItem icon={item.icon} size={24} color="#FFC24E" />}
